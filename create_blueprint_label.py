...
def createBlueprintLabels(projectRefs):
    ...
    blueprintResp = session.get(blueprintsUrl, params=queryParams, auth=basic)
    # Converts the response to json
    page = blueprintResp.json()["blueprints"]
    ...
    for projectRef in projectRefs:
        ...
        if(len(filtrado) > 0):
            blueprint = filtrado[0]
            labels = list(filter(lambda x: x.startswith(projectRef), blueprint["labels"]))
            if(len(labels) > 0):
                print("Blueprint já possui label {}".format(projectRef))
            else:
                blueprint["labels"] = [ projectRef ]
                # Patches blueprint with both azure connections and scopes
                resp = session.patch(blueprintUrl.format(str(blueprint["id"])), data = json.dumps(blueprint), auth=basic, headers = {'Content-type': 'application/json'})
                if(resp.status_code == 200):
                    queryParams = { "pageSize": 800, "page": 1 }
                    response = session.get(blueprintUrl.format(resp.json()["id"]), params=queryParams, auth=basic)
                    blueprint = response.json()
                    if(resp.status_code != 200):
                        print("Status Execução: {}-{}.{}".format(resp.status_code, blueprint["id"], resp))
                    else:
                        print("Blueprint '{}' criado com sucesso!".format(projectRef))
                else:
                    print("Falha ao realizar busca do Blueprint".format(resp.json()))
        else:
            print("Blueprint não encontrado para o projeto {}.".format(projectRef))

def run(projectRefs):
    try:
        createBlueprintLabels(projectRefs)
